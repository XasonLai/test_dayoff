				...
				@if(count($each_staff) != 0 )
					...
					<div class="form-group">
						...
						@foreach($each_staff as $key => $item)
							@if( $item->check == 0 )
								<tr>
									<td><input type="checkbox" name="check[]" value="{{$item->id}}" >{{$item->date}} </td>
									<td>{{$item->time_start}} 至 {{$item->time_end}}</td>
									<td>  {{$item->hours}} 小時 {{$item->minutes}} 分鐘 </td>
									<td><a href="/staffs/compensatory/update/{{$item->id}}" class="btn btn-info">同意</a></td>
								</tr>
							@endif
						@endforeach
						</table>
					...
					<div class="form-group">
						...
						<!-- <a href="/staffs/compensatory/update" class="btn btn-info">同意</a> -->
					</div>
